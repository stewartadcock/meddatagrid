...
namespace Fr.Medit.MedDataGrid.Controls.CustomMenu
{
  ...
  /// <summary>
  /// Description résumée de MEDGraphicsMenu.
  /// </summary>
  public class MEDGraphicsMenu : System.ComponentModel.Component, IMenu
  {
    ...
    /*
    private string RadioCheckIcon = "Checkmark.bmp";
    private string RadioUnCheckIcon = "UnCheckmark.bmp";
    private string CheckIcon = "Checkmark.bmp";
    private string UnCheckIcon = "UnCheckmark.bmp";
    */

    private string RadioCheckIcon = "Delete.ico";
    private string RadioUnCheckIcon = "Delete.ico";
    private string CheckIcon = "Delete.ico";
    private string UnCheckIcon = "Delete.ico";
    ...
    //         ownerdraw objects
    public virtual void MyMeasureItem(object sender, MeasureItemEventArgs e)
    {
    ...
    //         ownerdraw objects
    public virtual void MyDrawItem(object sender, DrawItemEventArgs e)
    {
    ...
    public virtual RectangleF MakeRectangleF(Rectangle rect)
    {
    ...
    public virtual void CreateLayout(RectangleF bounds)
    {
      ...
      ItemTextBounds = new RectangleF();
      ItemTextBounds.X = (float)BitmapWidth + HorizontalTextOffset;
      ItemTextBounds.Y = bounds.Y + (float)VerticalTextOffset;
      ItemTextBounds.Width = bounds.Width;
      ItemTextBounds.Height = bounds.Height;
    }
    ...
    public virtual void DrawBitmap(Graphics g, MenuItem item, DrawItemState itemState)
    {
      ...
      if (isChecked == true)
      {
        if (item.RadioCheck)
        {
          bmp = GetEmbeddedImage(this.RadioCheckIcon);
        }
        else
        {
          bmp = GetEmbeddedImage(this.CheckIcon);
        }
      }
      else
      {
        if (item.RadioCheck)
        {
          bmp = GetEmbeddedImage(this.RadioUnCheckIcon);
        }
        else
        {
          if (menuItemIconCollection.ContainsKey(item))
          {
            bmp = GetEmbeddedImage(menuItemIconCollection[item]);
          }
        }
      }
      ...
      // Free the resource.
      // SAA TODO: Is this really needed?  Desirable?
      bmp.Dispose();
    }
    ...
    public virtual void DrawBackground(Graphics g, DrawItemState itemState)
    {
      ...
      if (selected && !disabled)
      {
        if (SpecialBackColorSelected)
        {
          backBrush = new LinearGradientBrush(rectToPaint,
            MenuItemBackColorSelectedStart, MenuItemBackColorSelectedEnd, LinearGradientMode.Horizontal);
        }
        else
        {
      ...
      else
      {
        ...
        if (paintBitmapArea)
        {
          ...
          bitmapBrush = new LinearGradientBrush(BitmapBounds,
            Color.White, Color.Gray, LinearGradientMode.Horizontal);
          g.FillRectangle(bitmapBrush, BitmapBounds);
        }
      }
    ...
    public virtual void DrawText(Graphics g, MenuItem item, DrawItemState itemState)
    {
    ...
    private static Image GetEmbeddedImage(string p_Image)
    {
      ...
      {
        // SAA TODO: Fix broken icons.
        MEDLogManager.Log(LogLevels.Error, "Dodgy icon resource: " + ex.Message);
        image = null;
      }
